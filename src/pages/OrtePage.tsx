...
import { OrteViewDialog } from '@/components/dialogs/OrteViewDialog';
...
export default function OrtePage() {
  const [records, setRecords] = useState<Orte[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Orte | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Orte | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Orte | null>(null);
  ...
  async function handleCreate(fields: Orte['fields']) {
    await LivingAppsService.createOrteEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Orte['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateOrteEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Orte"
      subtitle={`${records.length} Orte im System`}
      action={
          ...
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('beschreibung')}>
                <span className="inline-flex items-center gap-1">
                  Beschreibung / Warum interessant?
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('bereits_besucht')}>
                <span className="inline-flex items-center gap-1">
                  Bereits besucht
                  {sortKey === 'bereits_besucht' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('besuchsdatum')}>
                <span className="inline-flex items-center gap-1">
                  Besuchsdatum
                  {sortKey === 'besuchsdatum' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('notizen_nach_besuch')}>
                <span className="inline-flex items-center gap-1">
                  Persönliche Notizen nach dem Besuch
                  {sortKey === 'notizen_nach_besuch' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{record.fields.oeffnungszeiten ?? '—'}</TableCell>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.bereits_besucht ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.bereits_besucht ? 'Ja' : 'Nein'}</span></TableCell>
                <TableCell className="text-muted-foreground">{formatDate(record.fields.besuchsdatum)}</TableCell>
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.notizen_nach_besuch ?? '—'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={18} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Orte. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
