...
const ORTE_FIELDS = [
  { key: 'ort_name', label: 'Name des Ortes', type: 'string/text' },
  ...
  { key: 'oeffnungszeiten', label: 'Öffnungszeiten', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung / Warum interessant?', type: 'string/textarea' },
  { key: 'fotos', label: 'Fotos', type: 'file' },
  { key: 'bewertung', label: 'Bewertung', type: 'lookup/radio', options: [{ key: 'rating_1', label: '⭐ 1 – Weniger beeindruckend' }, { key: 'rating_2', label: '⭐⭐ 2 – Ganz okay' }, { key: 'rating_3', label: '⭐⭐⭐ 3 – Gut' }, { key: 'rating_4', label: '⭐⭐⭐⭐ 4 – Sehr gut' }, { key: 'rating_5', label: '⭐⭐⭐⭐⭐ 5 – Absolut empfehlenswert' }] },
  { key: 'bereits_besucht', label: 'Bereits besucht', type: 'bool' },
  { key: 'besuchsdatum', label: 'Besuchsdatum', type: 'date/date' },
  { key: 'notizen_nach_besuch', label: 'Persönliche Notizen nach dem Besuch', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'kategorien', label: 'Kategorien', pascal: 'Kategorien' },
  { key: 'orte', label: 'Orte', pascal: 'Orte' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'orte': return (data as any).orte as Orte[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'orte' && fieldKey === 'kategorie') {
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'orte': return {
        create: (fields: any) => LivingAppsService.createOrteEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateOrteEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteOrteEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'orte' || dialogState?.entity === 'orte') && (
        <OrteDialog
          open={createEntity === 'orte' || dialogState?.entity === 'orte'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'orte' ? handleUpdate : (fields: any) => handleCreate('orte', fields)}
          defaultValues={dialogState?.entity === 'orte' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'orte' && (
        <OrteViewDialog
          open={viewState?.entity === 'orte'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'orte', record: r }); }}
          kategorienList={(data as any).kategorien ?? []}
        />
      )}
