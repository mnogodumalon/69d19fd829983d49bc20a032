...
import type { Kategorien, Orte, CreateKategorien, CreateOrte } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- ORTE ---
  static async getOrte(): Promise<Orte[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ORTE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Orte[];
    return enrichLookupFields(records, 'orte');
  }
  static async getOrteEntry(id: string): Promise<Orte | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ORTE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Orte;
    return enrichLookupFields([record], 'orte')[0];
  }
  static async createOrteEntry(fields: CreateOrte) {
    return callApi('POST', `/apps/${APP_IDS.ORTE}/records`, { fields: cleanFieldsForApi(fields as any, 'orte') });
  }
  static async updateOrteEntry(id: string, fields: Partial<CreateOrte>) {
    return callApi('PATCH', `/apps/${APP_IDS.ORTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'orte') });
  }
  static async deleteOrteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ORTE}/records/${id}`);
  }
